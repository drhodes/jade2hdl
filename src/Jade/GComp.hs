...
getSigsWithIdent gcomp ident = "GComp.getSigsWithIdent" <? do
  -- find the signals in gcomp that shares the ident. There might be
  -- more than one!
  flattened <- concatMapM Sig.flatten $ getSigs (removeTerms gcomp)
  filterM (flip Sig.hasIdent ident) flattened

getSigsWithIdentNoFlatten :: GComp -> String -> J [Sig]
getSigsWithIdentNoFlatten gcomp ident = "GComp.getSigsWithIdent" <? do
  -- find the signals in gcomp that shares the ident. There might be
  -- more than one!
...
name :: GComp -> J String
name comp@(GComp gid nodes) = "GComp.name" <? do
  let genNameLen = 10
